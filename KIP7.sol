...
contract KIP7 is KIP13, IKIP7 {
    ...
    mapping (address => uint256) private _frozen_balances;
    ...
    function balanceOf(address account) external view returns (uint256) {
        return _balances[account].add(_frozen_balances[account]);
    }
    ...
    function frozenBalance(address account) external view returns (uint256){
        return _frozen_balances[account];
    }
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(amount > 0, "KIP7: transfer zero amount");
    ...
    function _distribute(address sender, address recipient, uint256 amount) internal{
        require(sender != address(0), "KIP7: distribute from the zero address");
        require(recipient != address(0), "KIP7: distribute to the zero address");
        require(!recipient.isContract(), "KIP7: distribute to the contract address");
        
        _balances[sender] = _balances[sender].sub(amount);
        _frozen_balances[recipient] = _frozen_balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }
    
    function _unlock(address account, uint256 amount) internal {
        require(account != address(0), "KIP7: unlock from the zero address");
        _frozen_balances[account] = _frozen_balances[account].sub(amount);
        _balances[account] = _balances[account].add(amount);
        emit UnLock(account, amount);
    }
    
    function _dispossess(address account, uint256 amount) internal{
